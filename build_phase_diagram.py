...
samples = 10
speed = 1.
log_file_path = '/media/Data4/hattenberger_data/test_phase_3/'
...
def run_cmd(param: dict):
    cmd = ['python', 'simu_phase_diagram.py',
            '--swarm_config=../UavSwarmFish/config/phase_diagram.yaml',
    ...
    subprocess.run(cmd)
    print(f'Done [{param["count"]}]: {' '.join(cmd)}')
...
nb_run = len(y_atts)*len(y_alis)*samples
print(f'Starting {nb_run} simulations')
idx = 0
params = []
