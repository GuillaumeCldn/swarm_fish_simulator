...
class SwarmFish_Scenario(SwarmFish_Controller):
    ...
    def __init__(self, ARGS, env, view, start=True):
        ...
        if USE_PANEL_FLOW:
            ...
            for d in env.DRONE_IDS:
                pos = self.obs[str(d-1)]["state"][0:3]
                goal = self.wps[self.wps_idx]
                vehicle = pgflow.Vehicle(ID=str(d-1), position=pos, goal=goal)
        ...
        if SHOW_DIRECTION:
            ...
            for d in env.DRONE_IDS:
                ...
                self.flow_lines[str(d-1)] = self.view.add_line(np.zeros(3), np.zeros(3), color=(0.,1.,0.,1.))
    ...
    def update_action(self):
        ...
        for uav_id in range(self.num_drones):
            ...
            if SHOW_DIRECTION:
                self.view.move_line(self.directions[uav_name], state.pos, state.pos+speed[0:3])
                self.view.move_line(self.speeds[uav_name], state.pos, state.pos+state.speed)
                self.view.move_line(self.flow_lines[uav_name], state.pos, state.pos+np.hstack((self.flow, np.array([0.]))))

        # test waypoint reached
        dwps = [np.linalg.norm(self.wps[self.wps_idx][0:2] - states[str(i)].pos[0:2]) for i in range(self.num_drones)]
        if np.min(dwps) < 10.:
